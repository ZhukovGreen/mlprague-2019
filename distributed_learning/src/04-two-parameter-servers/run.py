...
parser.add_argument(
    "--job_and_task", type=str, required=True
)
...
if ":" not in job_and_task:
    print(
        'Use --job_and_task="job_name:task_index" notation.'
    )
    exit(1)

job_name, task_id = job_and_task.split(
    ":", maxsplit=2
)
task_id = int(task_id)
...
server = tf.train.Server(
    cluster, job_name=job_name, task_index=task_id
)
...
if job_name == "parameter_server":
    ...
    with tf.device(
        tf.train.replica_device_setter(
            cluster=cluster,
            worker_device=f"/job:worker/task:{task_id}",
        )
    ):
        global_step = (
    ...
    with tf.train.MonitoredTrainingSession(
        ...
        while not sess.should_stop():
            ...
            print(
                f"step: {result[2]}, a: {result[0]}, b: {result[1]}"
            )
