...
add = tf.add(a, b)
with tf.Session() as sess:
    first_result = sess.run(
        add, feed_dict={a: 1, b: 2}
    )
    print("1 + 2 =", first_result)
    second_result = sess.run(
        add, feed_dict={a: 3, b: 4}
    )
...
print(
    g.get_operations()
)  # => [1x const, 2x placeholder, 1x add] operations
...
with tf.Session(graph=graph_a) as sess:
    print(
        sess.run(constant_in_a)
    )  # => 'constant in graph a'

with tf.Session(graph=graph_b) as sess:
    print(
        sess.run(constant_in_b)
    )  # => 'constant in graph b'

print(
    g.get_operations()
)  # => default graph still only has the 4 originals
